...
def flatten_lead(gmaps_data: dict, contacts: dict, search_query: str) -> dict:
    ...
    lead_id = generate_lead_id(gmaps_data.get("title", ""), address)
...
def append_leads_to_sheet(worksheet, leads: list[dict], existing_ids: set) -> int:
    ...
    rows = [[lead.get(col, "") for col in LEAD_COLUMNS] for lead in new_leads]
...
def enrich_businesses(businesses: list[dict], max_workers: int = 3) -> list[dict]:
    ...
    for business in without_websites:
        enriched.append({"gmaps": business, "contacts": {"error": "No website available"}})
...
def run_pipeline(
    ...
    businesses = scrape_google_maps(search_query=search_query, max_results=max_results, location=location)
    ...
    leads = [flatten_lead(item["gmaps"], item["contacts"], search_query) for item in enriched]
    ...
    try:
        ...
        results["leads_added"] = append_leads_to_sheet(worksheet, leads, existing_ids)
    except Exception as e:
...
Examples:
  ...
  python3 gmaps_lead_pipeline.py --search "dentists" --location "Miami FL" --limit 25
