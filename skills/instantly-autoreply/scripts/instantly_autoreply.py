...
def generate_reply(
    ...
    if conversation_history:
        for email in conversation_history[:5]:
...
Otherwise, write the reply now:"""
    ...
    try:
        ...
        logger.info(f"Claude response: {reply_text[:500] if reply_text else 'EMPTY'}")
...
def run(payload: dict, token_data: dict, slack_notify=None) -> dict:
    ...
    if not campaign_id:
        ...
        campaign_id = campaign_name.split("|")[0].strip() if "|" in campaign_name else campaign_name.strip()

    if not campaign_id:
        notify("No campaign ID found, skipping")
    ...
    if not kb:
        notify(f"No knowledge base for campaign {campaign_id}, skipping")
    ...
    # Step 4: Generate reply with Claude
    notify("Generating reply with Claude...")
    reply = generate_reply(payload, kb, history)

    if not reply:
        notify("Claude decided to skip")
        return {"status": "success", "skipped": True, "reason": "claude_skip"}

    notify(f"Generated reply ({len(reply)} chars): {reply[:200]}...")
    ...
    return {"status": "success", "skipped": False, "reply_sent": True, "reply_length": len(reply)}
