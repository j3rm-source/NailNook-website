...
def extract_sheet_id(url):
    """Extract the Google Sheet ID from a URL or return as-is if already an ID."""
    if '/d/' in url:
...
def get_credentials():
    """Get OAuth2 credentials, refreshing or re-authorizing as needed."""
    scopes = [
...
def read_google_sheet(sheet_url, worksheet_name=None):
    ...
    Returns:
        List of dictionaries containing lead data, or None on error.
    """
    try:
...
def save_leads(leads, prefix="leads_input"):
    """Save leads to a timestamped JSON file in .tmp/."""
    if not leads:
